...
@RestController
@RequestMapping("/api")
public class ManagedUserResource {

    private final Logger log = LoggerFactory.getLogger(ManagedUserResource.class);
    ...
    /**
     * POST  /managed-users  : Creates a new user.
     * <p>
    ...
    public ResponseEntity<?> createUser(@RequestBody ManagedUserVM managedUserVM) throws URISyntaxException {
        ...
        if (userRepository.findOneByLogin(managedUserVM.getLogin().toLowerCase()).isPresent()) {
            ...
            return ResponseEntity.created(new URI("/api/managed-users/" + newUser.getLogin()))
    ...
    /**
     * PUT  /managed-users : Updates an existing User.
     *
    ...
    /**
     * GET  /managed-users : get all users.
     *
    ...
    public ResponseEntity<List<ManagedUserVM>> getAllUsers(@ApiParam Pageable pageable)
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/managed-users");
        return new ResponseEntity<>(managedUserVMs, headers, HttpStatus.OK);
    }

    /**
     * GET  /managed-users/:login : get the "login" user.
     *
     * @param login the login of the user to find
     * @return the ResponseEntity with status 200 (OK) and with body the "login" user, or with status 404 (Not Found)
     */
    @GetMapping("/managed-users/{login:" + Constants.LOGIN_REGEX + "}")
    @Timed
    @Secured(AuthoritiesConstants.ADMIN)
    ...
    /**
     * DELETE /managed-users/:login : delete the "login" User.
     *
     * @param login the login of the user to delete
     * @return the ResponseEntity with status 200 (OK)
     */
    @DeleteMapping("/managed-users/{login:" + Constants.LOGIN_REGEX + "}")
    @Timed
    @Secured(AuthoritiesConstants.ADMIN)
    ...
    /**
     * SEARCH  /_search/managed-users/:query : search for the User corresponding
     * to the query.
     *
     * @param query the query to search
     * @return the result of the search
     */
    @GetMapping("/_search/managed-users/{query}")
    @Timed
    @Secured(AuthoritiesConstants.ADMIN)
    public List<ManagedUserVM> search(@PathVariable String query) {
